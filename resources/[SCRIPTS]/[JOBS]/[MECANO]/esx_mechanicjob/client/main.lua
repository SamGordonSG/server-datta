    ...
    while messagenotfinish do
      ...
      DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", "", "", "", "", 200)
        ...
        if (GetOnscreenKeyboardResult()) then
           ...
           TriggerServerEvent('esx_mechanicjob:annonce',result)
		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					...
					{label = _U('towtruck'), value = 'towtruck2'},
					{label = _U('tow_truck'), value = 'towtruck'},
					{label = _U('sandkingxxxl'), value = 'sandkingxxxl'},
					{label = _U('sadler'), value = 'sadler'}
				}
				...
				if Config.EnablePlayerManagement and ESX.PlayerData.job and (ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'chief' or ESX.PlayerData.job.grade_name == 'experimente') then
					table.insert(elements, {label = 'sandkinghauler', value = 'sandkinghauler'})
					table.insert(elements, {label = 'pjtrailer', value = 'pjtrailer'})
					table.insert(elements, {label = 'trailersmall', value = 'trailersmall'})
				end
...
function OpenMechanicHarvestMenu()
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		local elements = {
			...
			{label = _U('alarm1'), value = 'alarm1'},
			{label = _U('alarm2'), value = 'alarm2'},
			{label = _U('alarm3'), value = 'alarm3'},
			{label = _U('alarminterface'), value = 'alarminterface'}
		...
		{css = 'metier',
			...
			elseif data.current.value == 'alarm1' then
				TriggerServerEvent('esx_mechanicjob:startHarvest4')
			elseif data.current.value == 'alarm2' then
				TriggerServerEvent('esx_mechanicjob:startHarvest5')
			elseif data.current.value == 'alarm3' then
				TriggerServerEvent('esx_mechanicjob:startHarvest6')
			elseif data.current.value == 'alarminterface' then
				TriggerServerEvent('esx_mechanicjob:startHarvest7')	
			end
		end, function(data, menu)
			menu.close()
...
function OpenMobileMechanicActionsMenu()
	...
	{css = 'metier',
		...
		elements = {
			...
			--{label = _U('sadlerrt'),      value = 'dep_vehicle'},
		...
		elseif data.current.value == 'dep_vehicle' then
			...
			if isVehicleTow then
				...
				if CurrentlyTowedVehicle == nil then
					if targetVehicle ~= 0 then
						if not IsPedInAnyVehicle(playerPed, true) then
							if vehicle ~= targetVehicle then
								...
								--AttachEntityToEntity(targetVehicle, vehicle, 20, 0.0, -3.5, 1.65, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
					...
					AttachEntityToEntity(CurrentlyTowedVehicle, vehicle, 20, -0.5, -13.0, -0.1, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
					--AttachEntityToEntity(CurrentlyTowedVehicle, vehicle, 20, -0.5, -12.0, -0.1, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
        ...
        while true do
            Citizen.Wait(1)    
                ...
                if (affichenews == true) then
                    ...
                    DrawAdvancedTextCNN(0.588, 0.14, 0.005, 0.0028, 0.8, "~r~ BENNY'S CUSTOMS ~d~", 255, 255, 255, 255, 1, 0)
...
RegisterNetEvent('esx_mechanicjob:annonce')
AddEventHandler('esx_mechanicjob:annonce', function(text)
    texteafiche = text
    affichenews = true
...
RegisterNetEvent('esx_mechanicjob:annoncestop')
AddEventHandler('esx_mechanicjob:annoncestop', function()
    affichenews = false
...
AddEventHandler('esx_mechanicjob:hasExitedMarker', function(zone)
	...
	elseif zone == 'Garage' then
		...
		TriggerServerEvent('esx_mechanicjob:stopHarvest7')
	end
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						if
							...
							--GetEntityModel(vehicle) == GetHashKey('sadlerrt')   or
							GetEntityModel(vehicle) == GetHashKey('sandkinghauler')   or
							GetEntityModel(vehicle) == GetHashKey('sandkingxxxl')   or
							GetEntityModel(vehicle) == GetHashKey('pjtrailer')   or
							GetEntityModel(vehicle) == GetHashKey('towtruck') or
							GetEntityModel(vehicle) == GetHashKey('towtruck2') or
							GetEntityModel(vehicle) == GetHashKey('trailersmall') or
							GetEntityModel(vehicle) == GetHashKey('sadler')
						then
