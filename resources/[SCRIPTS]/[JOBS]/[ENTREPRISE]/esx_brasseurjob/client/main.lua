	...
	ESX.UI.Menu.Open(
		...
		css = 'vestiaire',
			...
			elements = {
				...
				{label = _U('brasseur_clothes_brasseur'), value = 'brasseur_wear'},
			},
		},
		function(data, menu)
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 86.60, function(vehicle)
		...
		local elements = {
			{label = 'Camion Travail',  value = 'pounder'},
			{label = 'Voiture Patron',  value = 'dominator2'},
		}
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
		        ...
		        local platenum = math.random(1, 9999)
                local platePrefix = Config.platePrefix
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 86.60, function(vehicle)
					local playerPed = GetPlayerPed(-1)
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
					SetVehicleNumberPlateText(vehicle, platePrefix .. platenum)
					plate = GetVehicleNumberPlateText(vehicle)
					plate = string.gsub(plate, " ", "")
					TriggerServerEvent('esx_vehiclelock:registerkeyjob', plate, 'no')
				end)
			end,
			function(data, menu)
...
AddEventHandler('esx_brasseurjob:hasEnteredMarker', function(zone)
	...
	if zone == 'MaltFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'malt_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementMalt' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'malt_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementOrge' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'orge_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
	
	if zone == 'TraitementHoublon' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'houblon_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'farm_resell1'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end
	
	if zone == 'SellFarmA' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'farm_resell2'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end
	
	if zone == 'SellFarmB' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'farm_resell3'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end
...
AddEventHandler('esx_brasseurjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'HoublonFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur' then
		TriggerServerEvent('esx_brasseurjob:stopHarvest1')
	end
	if (zone == 'OrgeFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur' then
		TriggerServerEvent('esx_brasseurjob:stopHarvest2')
	end
	if (zone == 'MaltFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur' then
		TriggerServerEvent('esx_brasseurjob:stopHarvest3')
	end
	if (zone == 'TraitementHoublon' or zone == 'TraitementOrge' or zone == 'TraitementMalt') and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur' then
		TriggerServerEvent('esx_brasseurjob:stopTransform')
	end
	if (zone == 'SellFarm' or zone == 'SellFarmA' or zone == 'SellFarmB') and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur' then
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'houblon_harvest' then
					TriggerServerEvent('esx_brasseurjob:startHarvest1', CurrentActionData.zone)
				end
				if CurrentAction == 'orge_harvest' then
					TriggerServerEvent('esx_brasseurjob:startHarvest2', CurrentActionData.zone)
				end
			    if CurrentAction == 'malt_harvest' then
					TriggerServerEvent('esx_brasseurjob:startHarvest3', CurrentActionData.zone)
				end
				if CurrentAction == 'houblon_traitement' then
					TriggerServerEvent('esx_brasseurjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'malt_traitement' then
					TriggerServerEvent('esx_brasseurjob:startTransform', CurrentActionData.zone)	
				end
				if CurrentAction == 'orge_traitement' then
					TriggerServerEvent('esx_brasseurjob:startTransform', CurrentActionData.zone)	
				end
				if CurrentAction == 'farm_resell1' then
					TriggerServerEvent('esx_brasseurjob:startSell1', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell2' then
					TriggerServerEvent('esx_brasseurjob:startSell2', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell3' then
					TriggerServerEvent('esx_brasseurjob:startSell3', CurrentActionData.zone)
				end
