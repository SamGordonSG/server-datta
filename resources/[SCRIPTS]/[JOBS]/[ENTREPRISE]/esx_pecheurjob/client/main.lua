...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		...
		css = 'vestiaire',
			...
			align    = 'top-left',
			elements = {
				{label = _U('peche_clothes_civil'), value = 'citizen_wear'},
				{label = _U('peche_clothes_peche'), value = 'peche_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'peche_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'peche_actions_menu'
...
function OpenPecheActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'peche_actions',
		{
		css = 'vestiaire',
			title    = 'Peche',
			align    = 'top-left',
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'peche', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'peche', vehicleProps)
		...
		local elements = {
			{label = 'Fourgon de travail',  value = 'paradise'},
		}
		...
		if Config.EnablePlayerManagement and PlayerData.job ~= nil and (PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'chef' or PlayerData.job.grade_name == 'experimente') then
              table.insert(elements, {label = 'Véhicule Patron', value = 'bobcatxl'})
        end
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
		        ...
		        local platenum = math.random(100, 9999)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 56.326, function(vehicle)
					local playerPed = GetPlayerPed(-1)
					SetVehicleCustomPrimaryColour(vehicle, 0, 0, 255) --Peinture primaire
                    SetVehicleCustomSecondaryColour(vehicle, 0, 0, 255) --Peinture secondaire
					TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint2.Pos, 322.540, function(vehicle)
						ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
						local playerPed = GetPlayerPed(-1)
						TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
					end)
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint2.Pos, 322.540, function(vehicle)
					local playerPed = GetPlayerPed(-1)
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
				end)
			end,
...
function OpenMobilePecheActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_peche_actions',
		{css = 'metier',
			title    = 'pecheur',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_peche', 'Pecheur', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_pecheurjob:hasEnteredMarker', function(zone)
	if zone == 'SaumonFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'peche'  then
		CurrentAction     = 'saumon_harvest'
		CurrentActionMsg  = _U('press_collect1')
		CurrentActionData = {zone= zone}
	end

	if zone == 'ThonFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'peche'  then
		CurrentAction     = 'thon_harvest'
		CurrentActionMsg  = _U('press_collect2')
		CurrentActionData = {zone= zone}
	end

	if zone == 'CrevetteFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'peche'  then
		CurrentAction     = 'crevette_harvest'
		CurrentActionMsg  = _U('press_collect3')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementSaumon' and PlayerData.job ~= nil and PlayerData.job.name == 'peche'  then
		CurrentAction     = 'saumon_traitement'
		CurrentActionMsg  = _U('press_traitement1')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementThon' and PlayerData.job ~= nil and PlayerData.job.name == 'peche'  then
		CurrentAction     = 'thon_traitement'
		CurrentActionMsg  = _U('press_traitement2')
		CurrentActionData = {zone = zone}
	end
	if zone == 'TraitementCrevette' and PlayerData.job ~= nil and PlayerData.job.name == 'peche'  then
		CurrentAction     = 'crevette_traitement'
		CurrentActionMsg  = _U('press_traitement3')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'peche'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'PecheActions' and PlayerData.job ~= nil and PlayerData.job.name == 'peche' then
		CurrentAction     = 'peche_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'peche' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end

	if zone == 'VehicleSpawner2' and PlayerData.job ~= nil and PlayerData.job.name == 'peche' then
		CurrentAction     = 'vehicle_spawner_menu2'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'peche' then
	...
	if zone == 'VehicleDeleter2' and PlayerData.job ~= nil and PlayerData.job.name == 'peche' then
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.PecheActions.Pos.x, Config.Zones.PecheActions.Pos.y, Config.Zones.PecheActions.Pos.z)
		...
		AddTextComponentString("Société de Pêche")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'peche' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'peche' and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'peche_actions_menu' then
					OpenPecheActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'peche', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'peche' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobilePecheActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)