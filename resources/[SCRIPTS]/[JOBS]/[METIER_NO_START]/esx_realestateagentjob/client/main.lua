...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 248.550, function(vehicle)
		...
		local elements = {
			{label = 'Véhicule Employé',  value = 'washington'},
			{label = 'Véhicule Patron',  value = 'exemplar'},
		}
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
		        ...
		        local platenum = math.random(1, 999)
            local platePrefix = Config.platePrefix
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 248.550, function(vehicle)
...
AddEventHandler('esx_realestateagentjob:hasEnteredMarker', function(zone)
  ...
  if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'realestateagent' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'realestateagent' then
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'realestateagent' and (GetGameTimer() - GUI.Time) > 300 then
		    ...
		    if CurrentAction == 'vehicle_spawner_menu' then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'realestateagent', vehicleProps)
					end
