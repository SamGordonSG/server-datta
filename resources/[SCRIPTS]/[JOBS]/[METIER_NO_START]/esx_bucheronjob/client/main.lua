	...
	ESX.UI.Menu.Open(
		...
		css = 'vestiaire',
			...
			elements = {
				...
				{label = _U('bucheron_clothes'), value = 'bucheron_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'bucheron_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 159.377, function(vehicle)
						ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
						local playerPed = GetPlayerPed(-1)
						TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
					end)

					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'bucheron', vehicleProps)
		...
		local elements = {
			{label = 'Camion Employés',  value = 'hauler'},
			{label = 'Camion Patron',  value = 'phantom3'},
			{label = 'Remorque',  value = 'trailerlogs'},
			{label = '4x4 Patron',  value = 'rancherxl'},
		}
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
		        ...
		        local platenum = math.random(10, 999)
                local platePrefix = Config.platePrefix
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 159.377, function(vehicle)
					local playerPed = GetPlayerPed(-1)
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
...
function OpenMobileBucheronActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_bucheron_actions',
		{
		css = 'metier',
			title    = 'bucheron',
			align    = 'top-left',
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_bucheron', 'Bucheron', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_bucheronjob:hasEnteredMarker', function(zone)
	if zone == 'WoodFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'bucheron'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementWood' and PlayerData.job ~= nil and PlayerData.job.name == 'bucheron'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_traitement_wood')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementPlank' and PlayerData.job ~= nil and PlayerData.job.name == 'bucheron'  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'bucheron'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'BucheronActions' and PlayerData.job ~= nil and PlayerData.job.name == 'bucheron' then
		CurrentAction     = 'bucheron_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'bucheron' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'bucheron' then
...
AddEventHandler('esx_bucheronjob:hasExitedMarker', function(zone)
	...
	if (zone == 'TraitementPlank' or zone == 'TraitementWood') and PlayerData.job ~= nil and PlayerData.job.name == 'bucheron' then
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.BucheronActions.Pos.x, Config.Zones.BucheronActions.Pos.y, Config.Zones.BucheronActions.Pos.z)
		...
		AddTextComponentString("Société RVB")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'bucheron' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'bucheron' and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'bucheron', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'bucheron' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileBucheronActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)