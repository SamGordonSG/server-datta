-----------------------------------------
-- Created and modify by Dominic Stanfford
--AKA Anthony Maertens/Manta
-----------------------------------------
	...
	ESX.UI.Menu.Open(
		...
		css = 'vestiaire',
			...
			elements = {
				...
				{label = _U('orpailleur_clothes'), value = 'orpailleur_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'orpailleur_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'orpailleur_actions_menu'
...
function OpenOrpailleurActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'orpailleur_actions',
		{
		css = 'vestiaire',
			title    = 'Orpailleurs',
			align    = 'top-left',
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 50.57, function(vehicle)
						ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
						local playerPed = GetPlayerPed(-1)
						TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
					end)

					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'orpailleur', vehicleProps)
		...
		local elements = {
		    {label = '4x4 Employé',  value = 'rebel'},
			{label = '4x4 Patron',  value = 'bjxl'},
		}
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 50.57, function(vehicle)
					local playerPed = GetPlayerPed(-1)
					SetVehicleCustomPrimaryColour(vehicle, 0, 255, 255) --Peinture primaire
                    SetVehicleCustomSecondaryColour(vehicle, 0, 255, 255) --Peinture secondaire
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
...
function OpenMobileOrpailleurActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_orpailleur_actions',
		{
		css = 'metier',
			title    = 'Orpailleurs',
			align    = 'top-left',
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_orpailleur', 'Orpailleur', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_orpailleurjob:getStockItems', function(items)
		local elements = {}
		...
		{css='metier',
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_orpailleurjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_orpailleurjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		{css='metier',
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_orpailleurjob:putStockItems', itemName, count)
...
AddEventHandler('esx_orpailleurjob:hasEnteredMarker', function(zone)
	if zone == 'PepiteFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur'  then
		CurrentAction     = 'pepite_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementLingot' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur'  then
		CurrentAction     = 'lingot_traitement'
		CurrentActionMsg  = _U('press_traitement_raffine')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementPepite' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur'  then
		CurrentAction     = 'pepite_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'OrpailleurActions' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur' then
		CurrentAction     = 'orpailleur_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur' then
...
AddEventHandler('esx_orpailleurjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'PepiteFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur' then
		TriggerServerEvent('esx_orpailleurjob:stopHarvest')
	end  
	if (zone == 'TraitementPepite' or zone == 'TraitementLingot') and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur' then
		TriggerServerEvent('esx_orpailleurjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur' then
		TriggerServerEvent('esx_orpailleurjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.OrpailleurActions.Pos.x, Config.Zones.OrpailleurActions.Pos.y, Config.Zones.OrpailleurActions.Pos.z)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_orpailleurjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_orpailleurjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'pepite_harvest' then
					TriggerServerEvent('esx_orpailleurjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'pepite_traitement' then
					TriggerServerEvent('esx_orpailleurjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'lingot_traitement' then
					TriggerServerEvent('esx_orpailleurjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_orpailleurjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'orpailleur_actions_menu' then
					OpenOrpailleurActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'orpailleur', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleur' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileOrpailleurActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)