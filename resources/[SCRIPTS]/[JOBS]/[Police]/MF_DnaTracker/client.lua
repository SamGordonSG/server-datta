  ...
  while self.dS and self.cS do
    ...
    if self.CurJob == self.AmbulanceJob or self.CurJob == self.PoliceJob then
      ...
      if distA and distA < self.DrawTextDist then
        drawing = "ammo"
        self:DrawText3D(self.AmmoAnalyzePos.x,self.AmmoAnalyzePos.y,self.AmmoAnalyzePos.z, "Appuyez sur [~g~E~s~] pour analyser les douilles.")
...
function MFD:HandleDrawText(plyPed)
  ...
  if closestDist and closestDist < 5.0 then
    local str = ''
    if closest.obj == self.BloodObject then str = "[ ~r~ADN~s~ ] : Sang Humain : Appuyez sur [~r~ E ~s~] pour faire un prélèvement." else str = "[ ~r~PREUVES ~s~] : Douilles : Appuyez sur [~r~ E ~s~] pour ramasser."; end 
    ...
    if (IsControlJustPressed(1, Keys["E"]) or IsDisabledControlJustPressed(0, Keys["E"])) and (GetGameTimer() - self.timer) > 150 then
      ...
      ESX.TriggerServerCallback('MF_DnaTracker:PickupEvidenceS', function(canPickup)
        if not canPickup then ESX.ShowNotification("Vous ne pouvez plus en ramasser.")
        else
...
AddEventHandler('MF_DnaTracker:AnalyzeDNA', function()
  ...
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Blood_Info", { title = "Analyse ADN", align = 'left', elements = elements }, 
    function(data,menu)
      menu.close()
...
AddEventHandler('MF_DnaTracker:AnalyzeAmmo', function()
  ...
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), "AnalyzeAmmo", { title = "Analyse des douilles", align = 'left', elements = elements }, 
    function(data,menu)
      menu.close()
      ESX.UI.Menu.CloseAll()          
    end,    
