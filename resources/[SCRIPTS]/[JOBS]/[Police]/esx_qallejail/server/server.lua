			...
			if jailTime ~= nil then
				...
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "JUGE",  Firstname .. " " .. Lastname .. " est incarcéré pour: " .. args[3] }, color = { 249, 166, 0 } })
					end)
				end
			else
				TriggerClientEvent("esx:showNotification", src, "Le temps est invalide!")
			end
		else
			TriggerClientEvent("esx:showNotification", src, "Ce citoyen n\'est pas en ville!")
		end
	else
...
RegisterCommand("unjail", function(src, args)
	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			TriggerClientEvent("esx:showNotification", src, "Ce citoyen n\'est pas en ligne!")
		end
	else
		TriggerClientEvent("esx:showNotification", src, "Vous n\'êtes pas policier!")
	end
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "JUGE",  Firstname .. " " .. Lastname .. " est incarcéré pour: " .. jailReason }, color = { 249, 166, 0 } })
	end)
...
AddEventHandler("esx-qalle-jail:unJailPlayer", function(targetIdentifier)
	...
	TriggerClientEvent("esx:showNotification", src, xPlayer.name .. " Libéré!")
end)
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
	...
	TriggerClientEvent("esx:showNotification", src, "Merci, voici un peu d\'argent pour la bouffe!")
end)
