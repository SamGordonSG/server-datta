...
function JailLogin()
	...
	ESX.ShowNotification("La dernière fois que tu as fait une sieste tu étais en prison, tu dois finir ta peine!")
...
function UnJail()
	...
	ESX.ShowNotification("Vous êtes libre, soyez cool maintenant! Bonne chance!")
end
...
function InJail()
	...
	local JailPosition = Config.JailPositions["Cell"]
		while jailTime > 0 do
			...
			ESX.ShowNotification("Il vous reste " .. jailTime .. " minutes restantes en prison!")

			TriggerServerEvent("esx-qalle-jail:updateJailTime", jailTime)
			if GetDistanceBetweenCoords(GetEntityCoords(playerPed), JailPosition.x, JailPosition.y, JailPosition.z) > 3500 then
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Tu as déjà pris ce paquet!")
							end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Laissez paquet", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Emprisonner citoyen", value = "jail_closest_player" },
				{ label = "Libérer citoyen", value = "unjail_player" }
			}
		}, 
	function(data, menu)
            	...
            	if jailTime == nil then
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Vous devez remplir cette partie!")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("La cellule est vide!")
					return
				end
