...
RegisterNUICallback("ChooseCathrine", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Catherine est marquée sur votre GPS, allez la chercher.")
...
AddEventHandler("sawu_hookers:ChosenHooker", function(model)
    if HookerSpawned then
        exports['mythic_notify']:SendAlert('error', "Vous avez déjà choisi une escort!")
    else
        HookerSpawned = true
        CreateHooker(model)
        Citizen.CreateThread(function()
