...
AddEventHandler("esx_receptury:RequestStart", function(item_name, time)
    ...
    for k, Cords in pairs(currentItem.cords) do
        if IsPedInAnyVehicle(playerPed) then
            exports.pNotify:SendNotification({text = ('Désolé, vous ne pouvez pas être dans un véhicule.'), type = "error", layout = "centerLeft", timeout = 2000})
            do return end
        end
        if isActionStarted then
            exports.pNotify:SendNotification({text = ('Vous devez attendre.'), type = "error", layout = "centerLeft", timeout = 2000})
            do return end
        end
    ...
    if not isInLocation then
        exports.pNotify:SendNotification({text = ('Vous ne pouvez pas faire ca ici.'), type = "error", layout = "centerLeft", timeout = 2000})
    else
...
AddEventHandler("esx_receptury:StartAction", function(currentStep, currentAction)
    Citizen.CreateThread(function()
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                menu.close()
                exports.pNotify:SendNotification({text = ('Patientez envirrons 40 sec svp.'), type = "info", layout = "centerLeft", timeout = 5000})
                exports.pNotify:SendNotification({text = ("Pour annuler, appuyez sur <button type='button'>X</button>"), type = "info", layout = "centerLeft", timeout = 8000})
...
function stopEvent()
    ...
    x = nil
    isActionStarted = false
    exports.pNotify:SendNotification({text = ('Annulé.'), type = "info", layout = "centerLeft", timeout = 2000})
    exports.pNotify:SendNotification({text = currentItem.fail_msg, type = "error", layout = "centerLeft", timeout = 2500})
end
