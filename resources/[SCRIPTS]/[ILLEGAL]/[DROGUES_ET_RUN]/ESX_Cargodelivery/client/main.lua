...
local options = {
    ...
    scale = 0.4,
    font = 0,
    menu_title = "Nous vous attendions",
    menu_subtitle = "GoFast",
    color_r = 0,
    color_g = 128,
    color_b = 255,
}
	...
	while true do
		...
		local v = Config.CargoProviderLocation 
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour parler avec le ~y~ commissionaire")
			...
			if event_is_running then
				...
				if pVehicle == event_vehicle then
					...
					if delivery_point_distance < 50.0 then
					 ...
					 DrawMissionText("Retournez au véhicule!", 1000)
				end
			end
	end
end)
		...
		Citizen.Wait(5000)
				if event_is_running then
						...
						if IsPedDeadOrDying(GetPlayerPed(-1)) then
							ResetCargo()
							DisplayMissionFailed('Vous êtes mort!')
						end

						if GetVehicleEngineHealth(event_vehicle) < 20 and event_vehicle ~= nil then
							ResetCargo()
							DisplayMissionFailed('Le véhicule a été sérieusement endommagé.')
						end

						if event_time_passed > 1800 then
							ResetCargo()
							DisplayMissionFailed('Le GoFast a expiré.')
						end
...
function CargoMenu()
	ClearMenu()
    options.menu_title = "GoFast"
    for i = 1, #Config.Scenarios do
    	Menu.addButton("Acheter un GoFast - $" .. Config.Scenarios[i].CargoCost, "PurchaseCargo", i)
   	end
...
function AlertThePolice()
	...
	if event_is_running then
    	...
    	TriggerServerEvent('esx_addons_gcphone:startCall', 'police', ":anon: Attention, une voiture immatriculée [ " .. vehicle_plate .. " ] est entrain de commettre un délit!", PlayerCoords, {
    	...
    	for i = 1, #Config.AlertExtraSocieties do
    		...
    		if PlayerData.job.name ~= Config.AlertExtraSocieties[i] then
    			...
    			TriggerServerEvent('esx_addons_gcphone:startCall', Config.AlertExtraSocieties[i], ":anon: Attention, une voiture immatriculée [ " .. vehicle_plate .. " ] est entrain de commettre un délit!", PlayerCoords, {
				PlayerCoords = { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z },
				})
...
function PurchaseCargo(scenario)
	...
	if event_is_running == true then
		...
		drawNotification("Vous êtes déjà en plein GoFast.")
		goto done

	end

	print("Minimum de flics en ville: " .. Config.Scenarios[scenario].MinCopsOnline .. " ||  Acheter un GoFast:  " .. Config.Scenarios[scenario].CargoCost .. "")
	...
	ESX.TriggerServerCallback('esx_cargodelivery:getCopsOnline', function(police)
		...
		if police >= Config.Scenarios[scenario].MinCopsOnline then
			...
			if bought then
				...
				drawNotification("GoFast achetez avec succès.")
			...
			drawNotification("Il faut au minimum ~b~" .. Config.Scenarios[scenario].MinCopsOnline .. " flics ~w~ en ville.")
...
function DisplayMissionFailed(label)
	...
	TriggerEvent('esx:showNotification', '~r~Echec de la mission: ~w~' .. label)
