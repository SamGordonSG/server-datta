...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~] Braquer ~g~avec violence~w~ la banque",
        silentstart = "[~r~E~w~] Braquer ~g~silencieusement~w~ la banque",
        usecard = "[~r~E~w~] Utiliser le Pass",
        usethermal = "[~r~E~w~] Utiliser une charge thermique",
        usehack = "[~r~E~w~] Hacker la sécurité",
        uselockpick = "[~r~E~w~] Crocheter",
        usesearch = "[~r~E~w~] Chercher",
        lootcash= "[~r~E~w~] Récupérer de l'argent",
        lootgold = "[~r~E~w~] Récupérer de l'or",
        lootdia = "[~r~E~w~] Récupérer des diamants",
        card = "Utilisation du pass",
        thermal = "Plante la charge thermique",
        burning = "Faire fondre la serrure",
        lockpick = "Crochetage",
        using = "En cours",
        used = "Complet.",
        stage = "terminé.",
        search = "Recherche",
        hacking = "Hack",
        melted = "Serrure fondue.",
        hacked = "Hack terminé.",
        unlocked = "Porte déverrouillée.",
        nothing = "Rien trouvé.",
        found = "Vous avez trouvez un pass.",
        time = "Temps restant: "
    }
}
UTK.searchinfo = {
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            ...
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Vous n avez pas d'ordi portable.")
                                            end
                                        end, "laptop_h")
                                    end
                                end
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports["mythic_notify"]:SendAlert("error", "Vous n'avez pas le pass.")
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Vous n'avez pas de charge thermique.")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    exports['mythic_notify']:SendAlert("error", "Vous n'avez pas de crochet.")
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
function UTK:Plant()
    ...
    RequestModel("hei_p_f_bag_var7_bus_s")
    RequestNamedPtfxAsset("scr_ornate_heist")
    while not HasAnimDictLoaded("anim@heists@ornate_bank@thermal_charge") and not HasModelLoaded("hei_p_f_bag_var7_bus_s") and not HasNamedPtfxAssetLoaded("scr_ornate_heist") do
        Citizen.Wait(50)
    end
    ...
    local bag = CreateObject(GetHashKey("hei_p_f_bag_var7_bus_s"), loc.x, loc.y, loc.z,  true,  true, false)
...
function UTK:Hack()
    ...
    while not HasAnimDictLoaded(animDict)
        ...
        or not HasModelLoaded("hei_p_f_bag_var7_bus_s")
    ...
    local bag = CreateObject(GetHashKey("hei_p_f_bag_var7_bus_s"), targetPosition, 1, 1, 0)
    ...
    Citizen.Wait(2000)
    Brute()
    exports["mythic_notify"]:SendAlert("success", "Ouvrir mon ordinateur et aller sur HackConnect.exe")
...
function Loot(currentgrab)
    ...
    local baghash = GetHashKey("hei_p_f_bag_var7_bus_s")
	...
	GrabBag = CreateObject(GetHashKey("hei_p_f_bag_var7_bus_s"), GetEntityCoords(PlayerPedId()), true, false, false)
	...
	SetModelAsNoLongerNeeded(GetHashKey("hei_p_f_bag_var7_bus_s"))
end
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    exports["mythic_notify"]:SendAlert("success", "Trouver l IP...")
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    exports["mythic_notify"]:SendAlert("success", "Trouver le MDP...")
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    exports["mythic_notify"]:SendAlert("success", "Lancer BruteForce.exe")
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['mythic_notify']:SendAlert("error", "Vous avez 2 min 30 avant la libération de gaz toxiques.")
        UTK.starttimer = true
        UTK.vaulttime = 150
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            exports["mythic_notify"]:SendAlert("success", "Vous avez volé "..text)
        end)
    else
        exports['mythic_notify']:SendAlert("error", "Vous avez 2 min 30 avant la libération de gaz toxiques.")
        UTK.starttimer = true
        ...
        for k, v in ipairs(PoliceDoors) do
            ...
            if dst <= 50 then
                if v.locked then
                    ...
                    doortext = "[~r~E~w~] Deverrouiller la porter"
                    state = false
                elseif not v.locked then
                    FreezeEntityPosition(v.obj, false)
                    doortext = "[~r~E~w~] Fermer la porte"
                    state = true
                end
                if PlayerData.job.name == "police" then
                    if dst <= 1.5 then
