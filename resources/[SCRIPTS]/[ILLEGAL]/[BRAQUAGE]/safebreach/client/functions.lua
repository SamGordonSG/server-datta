...
TryToCrack = function(safeEntity)
	...
	ESX.TriggerServerCallback("safebreach:checkIfSafeIsBreachable", function(state)
		if state then
			...
			ESX.ShowNotification("Vous ne pouvez pas faire ca maintenant.")
		end
			...
			if IsControlJustPressed(0, 22) then
				if dialRotation == currentSafeCorrectPosition then
					...
					if #safeLocks == 1 then
						...
						TriggerServerEvent("esx_phone:send", "police", "Il y a un cambriolage dans l'épicerie. Position sur le GPS!", true, { ["x"] = x, ["y"] = y, ["z"] = z })
					end
					...
					if #safeLocks < Config.TotalLocks then
						...
						ESX.TriggerServerCallback("safebreach:safeBreached", function(breached)
							if breached then
								...
								ESX.ShowNotification("Vous avez ouvert le coffre!")
							else
								ESX.ShowNotification("Coffe déjà ouvert!")
							end
						end, {
							["store"] = currentStreetName,
							["doorCoords"] = {
					...
					ESX.ShowNotification("Mauvaise combinaison.")
				end
...
TakeSafeItem = function(itemEntity, safeData)
	...
	if DoesEntityExist(itemEntity) then
		...
		ESX.ShowNotification("Quelqu'un est déjà passé...")
	end
end
