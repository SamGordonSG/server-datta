...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('esx_vehicleshop:getCommercialVehicles', function(vehicles)
		...
		for i=1, #vehicles, 1 do
			returnPrice = ESX.Round(vehicles[i].price * 1.00)
...
function OpenShopMenu()
	...
	for i=1, #Categories, 1 do
		...
		for j=1, #categoryVehicles, 1 do
			...
			table.insert(options, vehicle.name .. ' <span style="color: green;text-align: right">$' .. vehicle.price .. '</span>')
		end
	...
	elseif zone == 'ResellVehicle' then
		...
		if IsPedSittingInAnyVehicle(playerPed) then
			...
			if GetPedInVehicleSeat(vehicle, -1) == playerPed then
				...
				resellPrice = math.floor(vehicleData.price / 100 * Config.ResellPercentage)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'cardealer' then
