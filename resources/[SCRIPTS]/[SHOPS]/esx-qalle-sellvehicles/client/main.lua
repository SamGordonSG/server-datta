...
function LoadSellPlace()
	Citizen.CreateThread(function()
		...
		AddTextComponentString("Voiture d'occasion")
		...
		while true do
			...
			if dstCheck <= 10.0 then
				...
				if dstCheck <= 4.2 then
					ESX.Game.Utils.DrawText3D(SellPos, "[E] Ouvrir le menu", 2.0)
					if IsControlJustPressed(0, 38) then
						if IsPedInAnyVehicle(ped, false) then
							OpenSellMenu(GetVehiclePedIsUsing(ped))
						else
							ESX.ShowNotification("Vous devez etre dans un ~g~véhicule")
						end
					end
				end
			end
					...
					if dstCheck <= 2.0 then
						...
						if IsControlJustPressed(0, 38) then
							if IsPedInVehicle(ped, Config.VehiclePositions[i]["entityId"], false) then
								...
								ESX.ShowNotification("Vous devez etre assis dans un ~g~vehicule~s~!")
							end
						end
					end
				end
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	if not buyVehicle then
		if price ~= nil then
			...
			table.insert(elements, { ["label"] = "Définir le prix- :-", ["value"] = "price" })
		end
	else
		table.insert(elements, { ["label"] = "Acheter " .. price .. " - :-", ["value"] = "buy" })

		if owner then
			table.insert(elements, { ["label"] = "Reprendre véhicule", ["value"] = "remove" })
		end
	end
	...
	function(data, menu)
		...
		elseif action == "sell" then
			...
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:isVehicleValid", function(valid)
				...
				if valid then
					DeleteVehicle(veh)
					ESX.ShowNotification("Vous avez mis le ~g~vehicule~s~ en vente pour - " .. price .. " :-")
					menu.close()
				else
					ESX.ShowNotification("Vous devez etre le ~r~propriétaire~s~ du ~g~vehicule!~s~ / C'est ~r~déjà~s~ " .. #Config.VehiclePositions .. " en vente!")
				end
		...
		elseif action == "buy" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					ESX.ShowNotification("Vous avez ~g~acheté~s~ le véhicule pour " .. price .. " :-")
					menu.close()
				else
					ESX.ShowNotification("Vous~r~n avez~s~ pas assez d argent, il manque " .. price - totalMoney .. " :-")
				end
		...
		elseif action == "remove" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					ESX.ShowNotification("Vous avez ~g~repris~s~ le véhicule")
					menu.close()
				end
