...
TryToFish = function()
    if IsPedSwimming(cachedData["ped"]) then return ESX.ShowNotification("Vous ne pouvez pas pêcher et nager en même temps.") end
    if IsPedInAnyVehicle(cachedData["ped"]) then return ESX.ShowNotification("Vous devez sortir de votre véhicule pour pêcher.") end

    if not HasItems({
        Config.FishingItems["rod"]["name"],
        Config.FishingItems["bait"]["name"]
    }) then return ESX.ShowNotification("Vous avez besoin d'une canne à pêche et d'un leurre pour pêcher.") end
    ...
    if waterValidated then
        ...
        ESX.ShowNotification("Vous devez regarder vers l'eau pour pêcher.")
    end
end
...
CastBait = function(rodHandle, castLocation)
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        ESX.ShowHelpNotification("Lancer votre ligne en appuyant sur ~INPUT_DETONATE~")

        if GetGameTimer() - startedCasting > 5000 then
            ESX.ShowNotification("Vous devez utiliser le leurre.")
    ...
    DrawBusySpinner("En attente qu'un poisson morde à l'hameçon...")
...
TryToCatchFish = function()
    ...
    while true do
        ...
        ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ dans la zone verte.")
...
IsInWater = function()
    ...
    DrawBusySpinner("Vérification du lieu de pêche...")
...
SellFish = function()
    ...
    }) then return ESX.ShowNotification("Vous n'avez pas de poissons dans votre bourriche.") end
    ...
    ESX.TriggerServerCallback("james_fishing:sellFish", function(sold, fishesSold)
        if sold then
            ESX.ShowNotification("Vous avez vendu x" .. fishesSold .. " pour un motant total de $" .. sold)
        else
            ESX.ShowNotification("Reesayez.")
        end
    end)
end
