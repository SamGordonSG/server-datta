...
AddEventHandler('esx-qalle-hunting:sell', function()
    ...
    if MeatQuantity > 0 or LeatherQuantity > 0 then
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vous avez vendu ' .. LeatherQuantity + MeatQuantity .. ' pour $' .. LeatherPrice * LeatherQuantity + MeatPrice * MeatQuantity)
    else
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vous n avez plus rien a vendre')
    end
