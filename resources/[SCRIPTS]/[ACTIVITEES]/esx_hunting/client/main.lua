...
ESX                             = nil
local Blips = {
	{ name="Chasse", id=141,x=-769.23773193359, y=5595.6215820313, z=33.48571395874},
	{ name="Vente chasse", id=141,x=-370.90875244141, y=277.51498413086, z=86.421989440918},
}
...
local Positions = {
	['StartHunting'] = { ['hint'] = '[E] commencer la chasse', ['x'] = -769.23773193359, ['y'] = 5595.6215820313, ['z'] = 33.48571395874 },
	['Sell'] = { ['hint'] = '[E] Sell', ['x'] = -370.90875244141, ['y'] = 277.51498413086, ['z'] = 86.421989440918 },
			...
			for index, value in pairs(Positions) do
				if value.hint ~= nil then
					...
					if OnGoingHuntSession and index == 'StartHunting' then
						value.hint = '[E] arreter la chasse'
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] depecer', 0.4)
								...
								if IsControlJustReleased(0, Keys['E']) then
									if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
										...
										ESX.ShowNotification('Vous devez utiliser le couteau')
									end
								end
...
function SlaughterAnimal(AnimalId)
	...
	ESX.ShowNotification('L animal faisait ' ..AnimalWeight.. 'kg')
