...
function startExercise(animInfo, pos)
    ...
    Citizen.CreateThread(function()
        while exercising do
            ...
            DrawText2D(0.505, 0.95, 1.0,1.0,0.33, "Appuyez sur ~g~[ESPACE]~w~ pour vous entrainer", 255, 255, 255, 255)
            DrawText2D(0.505, 0.975, 1.0,1.0,0.33, "Appuyez sur ~r~[SUPPR]~w~ pour arrêter de vous entrainer", 255, 255, 255, 255)
        end
    end)
...
RegisterCommand("motion", function()
    ...
    Citizen.CreateThread(function()
        while doingMotion do
            ...
            if motionProcent >= 100000 then
                doingMotion = false
                motionProcent = 0
                Notify("Vous avez ~g~fini~w~ votre session d'entrainnement.")
            end
        end
    end)
    ...
    if doingMotion then
        ...
        if motionTimesDone <= 2 then
            Notify("Vous avez ~y~commencé~w~ votre session d'entrainnement.")
            print(motionTimesDone)
        else
            Notify("Vous êtes trop ~r~fatigué~w~ pour faire ca!")
            doingMotion = false
        end
    else
        Notify("Vous avez ~r~arreté~w~ votre session d'entrainnement.")
    end
end)
