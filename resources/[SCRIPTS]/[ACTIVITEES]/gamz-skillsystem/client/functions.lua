...
UpdateSkill = function(skill, amount)
    ...
    if Config.Skills["Stamina"]["Current"] > 50 then
        SetPedMoveRateOverride(GetPlayerPed(-1), 1.1)
        SetRunSprintMultiplierForPlayer(PlayerId(), 1.1)
    elseif Config.Skills["Stamina"]["Current"] > 70 then
        SetPedMoveRateOverride(GetPlayerPed(-1), 1.1)
        SetRunSprintMultiplierForPlayer(PlayerId(), 1.2)
    elseif Config.Skills["Stamina"]["Current"] > 90 then
        SetPedMoveRateOverride(PlayerId(), 10.0)
        SetRunSprintMultiplierForPlayer(PlayerId(), 1.49)
    end

    if Config.Skills["Strength"]["Current"] > 50 then
        SetPlayerWeaponDamageModifier(PlayerId(), 20.0)
    elseif Config.Skills["Strength"]["Current"] > 70 then
        SetPlayerWeaponDamageModifier(PlayerId(), 30.0)
    elseif Config.Skills["Strength"]["Current"] > 90 then
