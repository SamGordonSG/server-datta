...
Citizen.CreateThread(function()
	TriggerEvent('chat:addSuggestion', '/speedoskin', 'change le skin du compteur', { {name='skin', help='the skin name'} } )
	TriggerEvent('chat:addSuggestion', '/speedoskins', 'montre les skins disponible')
	TriggerEvent('chat:addSuggestion', '/speedotoggle', 'activer le compteur')
end)
...
RegisterCommand('speedoskin', function(source, args, rawCommand)
	if args[1] and DoesSkinExist(args[1]) then
		...
		TriggerEvent('chat:addMessage', { args = { '^3ERREUR', 'skin inconnu' } })
	end
end, false)
...
RegisterCommand('speedoskins', function(source, args, rawCommand)
	...
	TriggerEvent('chat:addMessage', { args = { '^2Skins disponible', message } })
end, false)
