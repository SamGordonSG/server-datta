		...
		if IsPedSittingInAnyVehicle(PlayerPedId()) then 
			for i = 1, #vehicleWashStation do
				...
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords2[1], garageCoords2[2], garageCoords2[3], true ) < 5 then
					if mycie == false then
					HelpPromt('Appuyez sur ~INPUT_CONTEXT~ pour ~b~laver ~s~votre véhicule pour ~g~(25$)')
					if IsControlJustPressed(1, Key) then
						TriggerServerEvent('es_better_carwash:checkmoney')
					end
				else
					HelpPromt('Appuyez sur ~INPUT_CONTEXT~ pour ~o~arrêter')
					if IsControlJustPressed(1, Key) then
						mycie = false
						timer2 = false
    ...
    Citizen.CreateThread(function()
		while timer2 do
			...
			elseif (timer == 0) then
				...
				TriggerEvent("pNotify:SendNotification", {text = "Votre véhicule à été néttoyé! Pour 25$", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft"})
				StopParticleFxLooped(particles, 0)
				StopParticleFxLooped(particles2, 0)
				timer2 = false
			end
        ...
        while true do
			...
			if mycie == true then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords3[1], garageCoords3[2], garageCoords3[3], true ) < 4 then
					DrawText3D(garageCoords3[1], garageCoords3[2], garageCoords3[3] + 3, '~b~nettoyage... ~s~Temps:~b~ '.. timer ..' ~s~seconds.')
				end
				end
			end
		end
...
AddEventHandler('es_better_carwash:notenoughmoney', function (moneyleft)
	TriggerEvent("pNotify:SendNotification", {text = "Vous n avez pas assez d'argent sur vous, vous avez besoin de" .. moneyleft .."$ de plus.", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft"}) 
end)
