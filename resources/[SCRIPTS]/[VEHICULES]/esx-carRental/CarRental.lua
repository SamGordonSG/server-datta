...
Citizen.CreateThread(function()
	...
	pickupStation = { --Set the car rental locaitons here
		...
		{x = -1042.421, y = -2662.624, z = 12.830},		--Airport car rental place

		{x = -927.319, y = -167.355, z = 40.878},  -- Hawick près hopital

		{x = 1982.653, y = 3775.477, z = 31.180},  -- Sandy Shore

		{x = 8.690, y = 6330.33, z = 30.239}  -- Paleto Bay
	...
	WarMenu.CreateMenu('carRental', 'Location')

	WarMenu.CreateSubMenu('closeMenu', 'carRental', 'Etes vous sur?')

	WarMenu.CreateSubMenu('carPicker', 'carRental', 'Choisissez un vehicule | 1 day about ' .. rentalTimer ..  ' minutes')

	WarMenu.CreateSubMenu('carInsurance', 'carRental', 'Voulez-vous prendre une assurance?')

	WarMenu.CreateMenu('carReturn', 'Retour Véhicule')

	WarMenu.SetSubTitle('carReturn', 'Voulez vous rendre le vehicule?') 

	WarMenu.CreateMenu('arrestCheck', 'Location')

	WarMenu.SetSubTitle('arrestCheck', 'Etes vous incarcéré?')
	...
	while true do
		...
		if WarMenu.IsMenuOpened('carRental') then
			...
			WarMenu.SetSubTitle('carRental', 'Louer au meilleur prix')
		...
		elseif WarMenu.IsMenuOpened('carPicker') then
			...
			if WarMenu.Button('Kalahari | Paiement: $500 | par 5 minutes: $250') then

				SpawnVehicle("kalahari")

				TriggerServerEvent("chargePlayer", 500)

				ESX.ShowNotification("Vous avez payé 500$ pour la location.")
			...
			elseif WarMenu.Button('Caddy | Paiement: $250 | par 5 minutes: $150') then

				SpawnVehicle("caddy")

				TriggerServerEvent("chargePlayer", 250)

				ESX.ShowNotification("Vous avez payé 250$ pour la location.")
			...
			elseif WarMenu.Button('Faggio | Paiement: $100 | Par 5 minutes: $100') then

				SpawnVehicle("faggio")

				TriggerServerEvent("chargePlayer", 100)

				ESX.ShowNotification("Vous avez payé 100$ pour la location.")
		...
		elseif WarMenu.IsMenuOpened('carInsurance') then
			...
			if WarMenu.Button('Oui | $500') then

				TriggerServerEvent("chargePlayer", 500)

				damageInsurance = true

				ESX.ShowNotification("Merci d'avoir pris un contrat d'assurance")
		...
		if not isBlipCreated then 
			...
			for _, v in pairs(dropoffStation) do
				...
				pickupBlip = AddBlipForCoord(v.x, v.y, v.z)
      			...
      			AddTextComponentString("Retour Location")
	...
	while true do
		...
		if plate == " RENTAL " then
			...
			if (IsVehicleDamaged(currentVehicle) and damageInsurance == false and damageCharge == false and canBeCharged == true) then
				...
				ESX.ShowNotification("On vous a facturé 500$ pour les dégats. Avec l'assurance, ca aurait été gratuit!")

			elseif (damageInsurance == true and IsVehicleDamaged(currentVehicle) and damageCharge == false) then

				ESX.ShowNotification("Vous avez endommagé le véhicule mais l'assurance prend tout en charge.")
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if isBeingCharged == true then
			...
			ESX.ShowNotification("Vous avez été débité de " .. autoChargeAmount .. "$ pour la location. Ramenez le véhicule pour ne plus payer.")
...
function returnVehicle()
			...
			ESX.ShowNotification("Merci d'avoir ramené le véhicule. Revenez nous voir!")
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if(GetDistanceBetweenCoords(coords, 1799.8345947266, 2489.1350097656, -119.02998352051, true) < 2.75 and isInPrison == false) then
			...
			ESX.ShowNotification("La police nous a indiqué que vous étiez incarcéré.")

			ESX.ShowNotification("Nous avons annulé le contrat et récupéré le véhicule.")
