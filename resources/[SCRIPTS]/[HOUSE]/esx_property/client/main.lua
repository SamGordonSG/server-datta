...
AddEventHandler("esx_property:keysReceived",function(data)
	...
	ESX.ShowNotification(_U('keys_received'):format(data.name,data.ownerName))
end)
...
function OpenPropertyMenu(property)
	...
	if PropertyIsOwned(property) then
		...
		if not Config.EnablePlayerManagement then
			...
			table.insert(elements, {label = _U('rent').. ' $ ' ..ESX.Math.GroupDigits(property.price /200), value = 'rent'})
		end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property', {
		...
		elseif data.current.value == 'give_keys' then
			...
			for k,v in ipairs(ESX.Game.GetPlayers()) do
				if v~=PlayerId() then table.insert(elems,{label=GetPlayerName(v).." - "..GetPlayerServerId(v),name=GetPlayerName(v),value=GetPlayerServerId(v)}) end
			end
			...
			function(data1,menu1)
				if data1.current.value~="exit" and data1.current.value~="take_away_keys" then
					ESX.TriggerServerCallback("esx_property:giveKeys",function()
						ESX.ShowNotification(_U('gave_keys'):format(data1.current.name))
				...
				elseif data1.current.value=="take_away_keys" then
					...
					ESX.ShowNotification(_U("removed_keys_all"))
				end
				menu1.close()
