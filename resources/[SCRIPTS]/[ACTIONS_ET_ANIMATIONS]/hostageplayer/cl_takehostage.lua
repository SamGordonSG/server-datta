...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Vous avez besoin d'avoir un pistolet en main pour prendre quelqu'un en hotage!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 then
			...
			drawNativeNotification("Personne dans les parages inutile de prendre le vent en hotage!")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Appuyez sur [G] pour libérer l'otage, [H] pour le tuer")
