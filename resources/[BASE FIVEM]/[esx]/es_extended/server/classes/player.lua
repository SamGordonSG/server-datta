function CreateExtendedPlayer(player, accounts, inventory, job, org, loadout, name, lastPosition)
	...
	self.getLimit = function()
		local inventoryLimit = 0

		for k,v in ipairs(self.inventory) do
			inventoryLimit = inventoryLimit + (v.count * v.limit)
		end
	...
	self.canCarryItem = function(name, count)
		local currentLimit, itemLimit = self.getLimit(), ESX.Items[name].limit
		local newLimit = currentLimit + (itemLimit * count)

		return newLimit <= self.maxLimit
	end

	self.setMaxLimit = function(newLimit)
		self.maxLimit = newLimit
		TriggerClientEvent('esx:setMaxLimit', self.source, self.maxLimit)
	end
	...
	self.setOrg = function(org, gradeorg)
		...
		if ESX.DoesOrgExist(org, gradeorg) then
			...
			print(('es_extended: ignoring setOrg for %s due to org not found!'):format(self.source))
		end
	end
