TriggerEvent('es:addGroupCommand', 'tp', 'admin', function(source, args, user)
	...
	if x and y and z then
		...
		TriggerClientEvent('chatMessage', source, 'SYSTEM', {255, 0, 0}, 'Coordonnées non valides!')
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEME', 'Permissions Insuffisantes.' } })
end, {help = 'Teleport aux coordonnées', params = {{name = 'x', help = 'X coords'}, {name = 'y', help = 'Y coords'}, {name = 'z', help = 'Z coords'}}})
	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(args[2], args[3]) then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEME', 'Ce job n\'existe pas.' } })
			end

		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEME', 'Joueur Hors Ligne.' } })
		end
	else
		TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEME', 'Usage Invalide.' } })
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEME', 'Permissions Insuffisantes.' } })
	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesOrgExist(args[2], args[3]) then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEME', 'Cette Organisation n\'existe pas.' } })
			end
...
TriggerEvent('es:addGroupCommand', 'setmoney', 'admin', function(source, args, user)
	...
	if target and money_type and money_amount and xPlayer ~= nil then
		...
		elseif money_type == 'black' then
			...
			TriggerClientEvent('chatMessage', _source, 'SYSTEME', {255, 0, 0}, '^2' .. money_type .. ' ^0 n\'est pas un bon compte !')
			return
		end
	else
		TriggerClientEvent('chatMessage', _source, 'SYSTEME', {255, 0, 0}, 'Invalid arguments.')
		return
	end

	print('es_extended: ' .. GetPlayerName(source) .. ' a donné $' .. money_amount .. ' (' .. money_type .. ') to ' .. xPlayer.name)
