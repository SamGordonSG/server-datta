			...
			table.insert(tasks2, function(cb2)
				...
				MySQL.Async.fetchAll('SELECT job, job_grade, org, org_gradeorg, loadout, position FROM users WHERE identifier = @identifier', {
					...
					if ESX.DoesOrgExist(org, gradeorg) then
						...
						print(('es_extended: %s had an unknown org [org: %s, gradeorg: %s], setting as org!'):format(player.getIdentifier(), org, gradeorg))

						local org, gradeorg = 'unemployed2', '0'
						local orgObject, gradeorgObject = ESX.Org[org], ESX.Org[org].gradeorg[gradeorg]
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				TriggerEvent("esx:dropitemalert",xPlayer.name,foundItem.label,total)
			end
		end
	elseif type == 'item_money' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > playerCash or playerCash < 1) then
				...
				TriggerEvent("esx:dropmoneyalert",xPlayer.name,total)
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
