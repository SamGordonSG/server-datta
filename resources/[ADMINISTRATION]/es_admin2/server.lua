	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			if available or user.getGroup() == "superadmin" then
				...
				TriggerClientEvent('chatMessage', Source, "SYSTEM", {255, 0, 0}, "Vous n\'avez pas la permission de faire ceci")
			end
		end)
	end)
end)
			...
			TriggerEvent('es:canGroupTarget', user.getGroup(), groupsRequired[type], function(available)
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "slay" then TriggerClientEvent('es_admin:quick', id, type) end
						if type == "kick" then DropPlayer(id, 'Kické par le staff du server') end
					
						if type == "ban" then
							for k,v in ipairs(GetPlayerIdentifiers(id))do
								banUser(v)
							end
							DropPlayer(id, GetConvar("es_admin_banreason", "Vous avez été banni du server"))
						end
					else
						if not available then
							TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "Votre statut ne permet pas cette commande.")
						else
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
					...
					TriggerEvent("es:getAllGroups", function(groups)
						if(groups[GROUP])then
							TriggerEvent("es:setPlayerData", USER, "group", GROUP, function(response, success)
								...
								TriggerClientEvent('chatMessage', -1, "CONSOLE", {0, 0, 0}, "Joueur ^2^*" .. GetPlayerName(tonumber(USER)) .. "^r^0 est devenu ^2^*" .. GROUP)
							end)
						else
				...
				if(GetPlayerName(USER) == nil)then
					...
					if(GROUP ~= nil and GROUP > -1)then
						TriggerEvent("es:setPlayerData", USER, "permission_level", GROUP, function(response, success)
							if(true)then
								TriggerClientEvent('chatMessage', -1, "CONSOLE", {0, 0, 0}, "Niveau de permission de ^2" .. GetPlayerName(tonumber(USER)) .. "^0 est maintenant ^2 " .. tostring(GROUP))
							end
						end)	
					else
			...
			elseif t == "bank" then
				...
				TriggerClientEvent('chatMessage', source, 'SYSTEM', {255, 0, 0}, "superadmin requis pour faire ça")
			end
		end)
	end)	
end)
	...
	if commandName == 'setadmin' then
		...
		TriggerEvent("es:setPlayerData", tonumber(args[1]), "permission_level", tonumber(args[2]), function(response, success)
			...
			if(true)then
				...
				TriggerClientEvent('chatMessage', -1, "CONSOLE", {0, 0, 0}, "Niveau de permission de ^2" .. GetPlayerName(tonumber(args[1])) .. "^0 est maintenant de ^2 " .. args[2])
			end
		end)
			...
			if(groups[args[2]])then
				TriggerEvent("es:setPlayerData", tonumber(args[1]), "group", args[2], function(response, success)
					...
					if(true)then
						...
						TriggerClientEvent('chatMessage', -1, "CONSOLE", {0, 0, 0}, "Niveau de ^2^*" .. GetPlayerName(tonumber(args[1])) .. "^r^0 a été activé ^2^*" .. args[2])
					end
				end)
			else
	...
	elseif commandName == 'giverole' then
		...
		if(GetPlayerName(tonumber(args[1])) == nil)then
			...
			TriggerEvent("es:getPlayerFromId", tonumber(args[1]), function(user)
				...
				TriggerClientEvent("chatMessage", user.get('source'), "SYSTEM", {255, 0, 0}, "Vous avez le role : ^2" .. table.concat(args, " "))
			end)
	...
	elseif commandName == 'removerole' then
		...
		if(GetPlayerName(tonumber(args[1])) == nil)then
			...
			TriggerEvent("es:getPlayerFromId", tonumber(args[1]), function(user)
				...
				TriggerClientEvent("chatMessage", user.get('source'), "SYSTEM", {255, 0, 0}, "Un role a été retiré: ^2" .. table.concat(args, " "))
			end)
	...
	elseif commandName == 'setmoney' then
			...
			TriggerEvent("es:getPlayerFromId", tonumber(args[1]), function(user)
				if(user)then
					...
					RconPrint("Money set")
					TriggerClientEvent('chatMessage', tonumber(args[1]), "CONSOLE", {0, 0, 0}, "Votre argent a été modifié: ^2^*$" .. tonumber(args[2]))
				end
			end)
...
end, {help = "Montre vos permissions et quel grade vous êtes"})
...
end, {help = "Reporte un joueur ou une situation", params = {{name = "report", help = "Ce que vous voulez reporter"}}})
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				if(#reason == 0)then
					reason = "Kicked: Vous avez été kick du server"
				else
					reason = "Kicked: " .. table.concat(reason, " ")
				end

				TriggerClientEvent('chatMessage', -1, "SYSTEM", {255, 0, 0}, "Joueur ^2" .. GetPlayerName(player) .. "^0 a été kické pour(^2" .. reason .. "^0)")
...
end, {help = "Kick le joueur avec ou sans raison spécifiée", params = {{name = "userid", help = "ID du joueur"}, {name = "reason", help = "Raison du kick"}}})
...
TriggerEvent('es:addGroupCommand', 'announce', "admin", function(source, args, user)
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Permissions Insuffisantes!")
end, {help = "Annonce un message à tout le serveur", params = {{name = "announcement", help = "Le message à annoncer"}}})
...
TriggerEvent('es:addGroupCommand', 'freeze', "mod", function(source, args, user)
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Permissions Insuffisantes!")
end, {help = "Freeze ou DéFreeze un joueur", params = {{name = "userid", help = "ID du joueur"}}})
...
TriggerEvent('es:addGroupCommand', 'bring', "mod", function(source, args, user)
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "SYSTEM", {255, 0, 0}, "Vous avez été TP par ^2" .. GetPlayerName(source))
				TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Joueur ^2" .. GetPlayerName(player) .. "^0 a été TP")
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
		end
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Permissions Insuffisantes!")
end, {help = "TP un joueur vers vous", params = {{name = "userid", help = "ID Du joueur"}}})
...
TriggerEvent('es:addGroupCommand', 'slap', "admin", function(source, args, user)
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "SYSTEM", {255, 0, 0}, "Vous avez été giflé par ^2" .. GetPlayerName(source))
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Permissions Insuffisantes!")
end, {help = "Gifler un joueur", params = {{name = "userid", help = "ID du joueur"}}})
...
TriggerEvent('es:addGroupCommand', 'goto', "mod", function(source, args, user)
		...
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				if(target)then
					...
					TriggerClientEvent('chatMessage', player, "SYSTEM", {255, 0, 0}, "Vous avez été TP ici by ^2" .. GetPlayerName(source))
					TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "TP sur joueur ^2" .. GetPlayerName(player) .. "")
				end
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Permissions Insuffisantes!")
end, {help = "TP sur joueur", params = {{name = "userid", help = "ID du joueur"}}})
...
TriggerEvent('es:addGroupCommand', 'crash', "superadmin", function(source, args, user)
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Joueur ^2" .. GetPlayerName(player) .. "^0 a été crash.")
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
		end
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Permissions Insuffisantes!")
end, {help = "Crash un joueur", params = {{name = "userid", help = "ID du joueur"}}})

-- Position
TriggerEvent('es:addGroupCommand', 'pos', "owner", function(source, args, user)
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Cette commande a été retiré")
end, function(source, args, user)
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Cette commande a été retiré")
end, {help = "Enregistre la position"})
