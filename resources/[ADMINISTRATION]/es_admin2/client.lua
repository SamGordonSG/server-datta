...
AddEventHandler('es_admin:quick', function(t, target)
	...
	if t == "crash" then 
		Citizen.Trace("On t'a fait crash! Maintenant tu sauras que ce serveur ne t'aime pas!\n")
		Citizen.CreateThread(function()
			while true do end
		end) 
	end
	...
	if t == "noclip" then
		...
		TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, "Mode NoClip a été ^2^*" .. msg)
	end
	if t == "freeze" then
...
AddEventHandler('es_admin:givePosition', function()
	...
	TriggerEvent('chatMessage', 'SYSTEM', {255, 0, 0}, 'Position sauvegardée.')
end)
