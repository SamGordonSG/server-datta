...
Citizen.CreateThread(function()
  	...
  	while true do
		...
		if InSpectatorMode then
			...
			if IsControlPressed(2, 117) then
...
-- taken from Easy Admin (thx to Bluethefurry)  --
			...
			if targetGod then
				table.insert(text,"Godmode: ~r~Trouvé~w~")
			else
				table.insert(text,"Godmode: ~g~Non Trouvé~w~")
			end
			...
			-- health info
			table.insert(text,"Santé"..": "..GetEntityHealth(targetPed).."/"..GetEntityMaxHealth(targetPed))
